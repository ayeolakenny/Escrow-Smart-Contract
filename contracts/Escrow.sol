...
contract Escrow {
    ...
    function myItem() public view returns (ItemStruct[] memory) {
    ...
    function requestItem(uint itemId) public returns (bool) {
        ...
        emit Action(itemId, "ITEM REQUESTED", Status.OPEN, msg.sender);
        return true;
    }
    ...
    function approveRequest(
        ...
        require(isAvailable[itemId] == Available.YES, "Item not available");
        require(requested[supplier][itemId], "Buyer not on the list");

        items[itemId].supplier = supplier;
        items[itemId].status = Status.PENDING;
        isAvailable[itemId] = Available.NO;

        emit Action(itemId, "ITEM APPROVED", Status.PENDING, msg.sender);
        return true;
    }

    function performDelivery(uint itemId) public returns (bool) {
        require(
            msg.sender == items[itemId].supplier,
            "You are not the approved buyer"
        );
        require(items[itemId].provided, "You have already delivered this item");
        require(
            items[itemId].confirmed,
            "You have already confirmed this item"
        );
    ...
    function confirmDelivery(uint itemId, bool provided) public returns (bool) {
        require(msg.sender == ownerOf[itemId], "Only owner allowed");
        require(items[itemId].provided, "You have not delivered this item");
        require(
            items[itemId].status != Status.REFUNDED,
            "Already refunded, create a new Item instead"
        );
        ...
        if (provided) {
            ...
            emit Action(itemId, "ITEM CONFIRMED", Status.CONFIRMED, msg.sender);
        } else {
            items[itemId].status = Status.DISPUTED;
            emit Action(itemId, "ITEM DISPUTED", Status.DISPUTED, msg.sender);
        }
        return true;
    }
